...
const WorldView = ({ data, setData, role, updateCloud, generateLoc, updateMapState }) => {
    const [tab, setTab] = useState('atlas'); // 'atlas' or 'map'
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex border-b border-slate-700 bg-slate-900 shrink-0">
                <button onClick={() => setTab('atlas')} className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 ${tab==='atlas' ? 'text-amber-500 border-b-2 border-amber-500' : 'text-slate-500'}`}>
                    <Icon name="book" size={16}/> Atlas & Bible
                </button>
                <button onClick={() => setTab('map')} className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 ${tab==='map' ? 'text-amber-500 border-b-2 border-amber-500' : 'text-slate-500'}`}>
                    <Icon name="map" size={16}/> Map Board
                </button>
            </div>

            <div className="flex-1 overflow-hidden relative">
                {tab === 'map' ? (
                    <MapBoard data={data} role={role} updateMapState={updateMapState} />
                ) : (
                    <div className="max-w-7xl mx-auto p-4 md:p-6 h-full flex flex-col md:flex-row gap-6 overflow-y-auto custom-scroll">
                        ...
                        <div className="w-full md:w-1/3 flex flex-col glass-panel rounded-xl overflow-hidden shrink-0">
                            ...
                            <div className="p-4 space-y-4">
                                {/* RESTORED LOCATION EDITOR */}
                                <div>
                                    <label className="text-xs font-bold text-green-400 uppercase flex items-center gap-1 mb-1">
                                        <Icon name="map-pin" size={12}/> Current Party Location
                                    </label>
                                    <input 
                                        disabled={role!=='dm'} 
                                        value={currentLocation} 
                                        onChange={e => updateCurrentLocation(e.target.value)} 
                                        placeholder="Where are they now?"
                                        className="w-full bg-slate-900 border border-green-900/50 rounded px-2 py-2 text-sm text-green-100 focus:border-green-500 outline-none font-bold"
                                    />
                                </div>
                            ...
                            {role === 'dm' && (
                                <div className="p-4 bg-slate-900/30 border-b border-slate-700 flex flex-col md:flex-row gap-2">
                                    <div className="flex gap-2 flex-1">
                                        ...
                                        <datalist id="loc-types"><option value="Town"/><option value="Dungeon"/><option value="Region"/><option value="Shop"/><option value="Tavern"/></datalist>
                                        <input value={genNote} onChange={e => setGenNote(e.target.value)} placeholder="Theme / Note" className="flex-1 bg-slate-800 border border-slate-600 rounded px-3 py-2 text-sm text-white outline-none"/>
                                    </div>
