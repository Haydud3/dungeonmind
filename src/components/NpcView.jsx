...
const NpcView = ({ data, setData, role, updateCloud, generateNpc, setChatInput, setView, onPossess }) => {
    ...
    const safeText = (val) => {
        if (typeof val === 'object' && val !== null) {
            if(val.primary) return val.primary; // Handle class objects
            ...
            <div className={`${selectedNpc ? 'flex' : 'hidden md:flex'} flex-1 glass-panel rounded-xl p-4 md:p-6 overflow-y-auto custom-scroll relative flex-col bg-slate-900/50`}>
                ...
                {selectedNpc ? (
                    <div className="space-y-4 pt-10 md:pt-0">
                        ...
                        {/* PERSONALITY EDITOR - FULLY UNLOCKED */}
                        <div className="mt-2">
                ...
                ) : (<div className="h-full flex flex-col items-center justify-center text-slate-500"><Icon name="skull" size={48} className="mb-4 opacity-20"/><p>Select or Generate an NPC.</p></div>)}
            </div>
        </div>
    );
};
