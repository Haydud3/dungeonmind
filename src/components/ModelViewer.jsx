...
const ModelViewer = ({ modelUrl, scale = 1, yOffset = 0, hideBaseIf3D = true, materialStyle }) => {
    // We use a simulated safeSize of 1.0 to match the typical VTT grid size ratio
    const safeSize = 1.0;
    const showModel = !!modelUrl;
    return (
        <Canvas camera={{ position: [0, 1.5, 2.5], fov: 45 }} shadows>
            <ambientLight intensity={0.4} color="#ffffff" />
            <directionalLight position={[5, 10, 5]} intensity={1.5} castShadow shadow-mapSize={[1024, 1024]} shadow-bias={-0.0001} shadow-normalBias={0.05} />
            <directionalLight position={[-5, 5, -5]} intensity={0.7} color="#90b0d0" />
            <directionalLight position={[0, 5, -10]} intensity={0.5} color="#ffe0b2" />
            ...
            <Suspense fallback={null}>
                ...
                <group position={[0, 0.04, 0]}>
                    ...
                    <group position={[0, 0.0075 + (yOffset * safeSize), 0]}>
                        <CharacterModel modelUrl={modelUrl} scale={scale * safeSize} materialStyle={materialStyle} />
                    </group>
                </group>

                {/* Ground plane reference */}
                <Grid infiniteGrid fadeDistance={5} sectionColor="#475569" cellColor="#334155" />
                <ContactShadows resolution={512} scale={10} blur={2} opacity={0.5} far={10} color="#000000" position={[0, 0, 0]} />
            </Suspense>
            
            <OrbitControls minPolarAngle={0} maxPolarAngle={Math.PI / 2 + 0.1} enablePan={false} autoRotate autoRotateSpeed={1.0} />
        </Canvas>
    );
};
