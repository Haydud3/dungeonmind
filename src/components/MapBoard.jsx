...
const MapBoard = ({ data, role, updateMapState }) => {
    ...
    const [mode, setMode] = useState('reveal'); // 'reveal' or 'shroud'
    ...
    useEffect(() => {
        ...
        // Resize canvas to fit image/container aspect ratio
        const resize = () => {
    ...
    }, [mapUrl]); // Re-init if map changes

    // Draw the Fog based on synced paths
    const drawFog = () => {
        const canvas = canvasRef.current;
        const ctx = canvas?.getContext('2d');
    ...
    // Redraw whenever paths update from cloud
    ...
    const getCoords = (e) => {
        ...
        // Return normalized (0-1) coordinates for syncing across screen sizes
        return {
        ...
        <div className="flex flex-col h-full bg-slate-900 overflow-hidden relative">
            ...
            {role === 'dm' && (
                <div className="p-2 bg-slate-800 border-b border-slate-700 flex gap-4 items-center shrink-0 z-20">
                    <label className="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded text-xs cursor-pointer flex items-center gap-2">
                        <Icon name="upload" size={14}/> Upload Map
                        <input type="file" accept="image/*" className="hidden" onChange={handleUpload}/>
                    </label>
                    <div className="h-6 w-px bg-slate-600"></div>
                    <button onClick={() => setMode('reveal')} className={`p-2 rounded ${mode==='reveal' ? 'bg-amber-600 text-white' : 'text-slate-400'}`} title="Reveal (Erase Fog)"><Icon name="eraser" size={18}/></button>
                    <button onClick={() => setMode('shroud')} className={`p-2 rounded ${mode==='shroud' ? 'bg-slate-600 text-white' : 'text-slate-400'}`} title="Shroud (Add Fog)"><Icon name="brush" size={18}/></button>
            ...
            <div ref={containerRef} className="flex-1 relative overflow-hidden flex items-center justify-center bg-black">
                {mapUrl ? (
                    <>
                        <img src={mapUrl} className="absolute inset-0 w-full h-full object-contain pointer-events-none select-none" alt="Map" />
                        <canvas 
                            ref={canvasRef}
                            className="absolute inset-0 w-full h-full fog-canvas"
